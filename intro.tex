...
So we see that we can obtain
the Lebesgue measure~$\Lmu$
as the convexification of the completion of~$\Smu$
relative to~$\wp(\R)$.
Similarly,
the Lebesgue integral~$\Lphi$
is the convexification of the completion of~$\Sphi$
relative to~$\smash{\E^\R}$,
where~$\Sphi\colon \SF\ra \R$ is the obvious
valuation on the set of simple functions~$\SF$ (see Example~\ref{E:sint-val}).
